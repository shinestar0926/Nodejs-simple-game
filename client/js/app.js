...
if(typeof screenNameForm==='undefined'){var xhr=new XMLHttpRequest();xhr.open('GET','html/start_screen.html',true);xhr.onreadystatechange=function(){if(this.readyState!==4)return;if(this.status!==200)return;var node=document.createElement('div');node.setAttribute("id","start-screen-content");node.innerHTML=this.responseText;document.body.appendChild(node);document.getElementById("button-play").onclick=beginGame;document.getElementById("button-spectate").onclick=spectate;};xhr.send();}else{document.body.appendChild(screenNameForm);document.getElementById("button-play").onclick=beginGame;document.getElementById("button-spectate").onclick=spectate;}}//set up the socket and begin talking with the server
function beginGame(){socket=socketIoClient();setupPlaySocket(socket);init();}function spectate(){socket=socketIoClient();setupSpectateSocket(socket);init();}function init(){//socket says it is ready to start playing.
...
screenNameForm=document.getElementById("start-screen-content");screenNameForm.parentNode.removeChild(screenNameForm);canvasGameBoard=new Canvas();drawingUtil=new DrawingUtil(canvasGameBoard.getCanvas());document.getElementById("leaderboard").style.display="block";document.getElementById("boost").style.display="block";startGame();}/**
	 * Basically this function lets us set up some global properties before the animation loop begins,
